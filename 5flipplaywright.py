#(searching product- oven,bed,laptoptable,skincare, toy, jewellary ,hairdryers, monitors ,music, sarees ,book ,MobileAccessories)
...
class ScraperConfig:
    ...
    def __init__(self):
        ...
        self.timeout: int = 20000  # Playwright uses milliseconds
        self.min_delay: float = 2.0
        self.max_delay: float = 4.0
        self.retry_attempts: int = 3
        self.retry_delay: float = 2.5
        ...
        # Multi-level fallback selectors for robustness
        self.selectors = {
            'product_container': 'div[data-id]',
            'title': [
                'a.KrRmtj', 'a.wfMR5l', 'a.VJA3rP', 
                'div.KzDlHZ', 'a[title]', 'div[class*="title"]'
            ],
            'brand': [
                'div.Fo1I0b', 'div._1UqcYF', 'div[class*="brand"]',
                'span.brand', 'div.brand'
            ],
            'current_price': [
            ...
            'rating': [
                'div.XQDdHH', 'div._3LWZlK', 'span[class*="rating"]'
            ],
            'rating_count': [
...
class RobustFlipkartScraper:
    """Production-ready Flipkart scraper using Playwright"""
    ...
    def __init__(self, config: Optional[ScraperConfig] = None):
        ...
        logger.info("✅ Robust scraper initialized")
    
    def _initialize_browser(self):
        """Initialize stealth browser with Playwright"""
        try:
            ...
            self.context.route("**/*.{png,jpg,jpeg,gif,svg,webp}", lambda route: route.abort())
            ...
            logger.info("🌐 Stealth browser initialized with Playwright")
    ...
    def search(self, query: str, max_pages: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        base_url = f"https://www.flipkart.com/search?q={quote_plus(query)}"
        all_products = []
        ...
        try:
            for page_num in range(1, max_pages + 1):
                ...
                try:
                    page_products = self._scrape_page(base_url, page_num)
                    all_products.extend(page_products)
                    
                    # Progress checkpoint
                    if len(all_products) % self.config.save_interval == 0:
                        self._save_checkpoint(all_products, query, page_num)
                    
                    # Early exit detection
                    if len(page_products) < 15:
                        logger.info("⚠️ Low product count, ending search")
                        break
    ...
    def _scrape_page(self, base_url: str, page_num: int) -> List[Dict[str, Any]]:
        """Scrape single page with smart loading"""
        ...
        try:
            ...
            self.page.wait_for_selector(
                self.config.selectors['product_container'],
            ...
            # Extract products
            products = self._extract_products(page_num)
            ...
            logger.warning(f"⏱️ Timeout on page {page_num}")
            return []
        except Exception as e:
    ...
    def _smart_scroll(self):
        ...
        while scroll_attempts < 5:
            ...
            if new_height == last_height:
                ...
                if not self.page.locator("div._2bnFzA").count():
                    break
    ...
    def _extract_products(self, page_num: int) -> List[Dict[str, Any]]:
        ...
        products = []
        elements = self.page.locator(self.config.selectors['product_container']).all()
    ...
    def _parse_product(self, element, page_num: int) -> Optional[FlipkartProduct]:
        """Parse product with intelligent fallbacks"""
        try:
            # Product ID
            product_id = element.get_attribute("data-id")
            if not product_id:
                # Fallback: generate from title hash
                title_elem = element.locator("a").first
                if title_elem.count():
                    product_id = f"pid_{hash(title_elem.inner_text()) % 100000}"
                else:
                    return None
            ...
            # Stock status
            out_of_stock_selector = ','.join(self.config.selectors['out_of_stock'])
            in_stock = element.locator(out_of_stock_selector).count() == 0
    ...
    def _get_text_with_fallbacks(self, element, selectors: List[str]) -> str:
        """Try multiple selectors intelligently"""
        for selector in selectors:
            try:
                elem = element.locator(selector).first
                if elem.count():
                    # Try title attribute first
                    attr_text = elem.get_attribute("title")
                    if attr_text:
    ...
    def _get_price_with_fallbacks(self, element, selectors: List[str]) -> int:
        """Extract price with sanitization"""
        for selector in selectors:
    ...
    def _get_product_url(self, element) -> str:
        """Extract canonical product URL"""
        try:
            link_elem = element.locator(self.config.selectors['link']).first
            if link_elem.count():
    ...
    def _get_image_url(self, element) -> str:
        ...
        for selector in self.config.selectors['image']:
            try:
                img_elem = element.locator(selector).first
                if img_elem.count():
                    src = img_elem.get_attribute("src")
                    if src:
                        return src.replace("200/200", "400/400")  # Higher resolution
            except:
                continue
        return ""
    ...
    def display_summary(self, products: List[Dict]):
        ...
        avg_discount = sum(p.get('discount', 0) for p in products) / len(products)
    ...
    def close(self):
        ...
        try:
            ...
            logger.warning(f"⚠️ Close error: {e}")
...
def interactive_mode() -> tuple[str, ScraperConfig, Optional[str]]:
    """Professional CLI"""
    print("\n" + "🛒"*40)
    print("✨ Flipkart Professional Scraper v2.0 (Playwright) ✨".center(80))
    print("🛒"*40)
